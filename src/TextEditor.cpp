...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Python()
{
    ...
    if (!inited)
    {
        ...
        // Common Python built-in functions and objects
        static const char* const identifiers[] = {
            "abs", "all", "any", "bin", "bool", "bytes", "chr", "complex", "dict",
            "dir", "divmod", "enumerate", "filter", "float", "format", "getattr",
        ...
        // Tokenization function for Python
        langDef.mTokenize = [](const char * in_begin, const char * in_end, const char *& out_begin, const char *& out_end, PaletteIndex & paletteIndex) -> bool
        {
            paletteIndex = PaletteIndex::Max;
            ...
            // Python string literals (both single and double quotes)
            else if ((*in_begin == '"' || *in_begin == '\'') && TokenizeCStyleString(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::String;
            // Python numbers
            else if (TokenizeCStyleNumber(in_begin, in_end, out_begin, out_end))
                paletteIndex = PaletteIndex::Number;
        ...
        langDef.mCommentStart = "\"\"\"";    // Triple quotes for multi-line strings/comments
        langDef.mCommentEnd = "\"\"\"";
        langDef.mSingleLineComment = "#";
