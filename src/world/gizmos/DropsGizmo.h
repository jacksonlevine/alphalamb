...
class DropsGizmo : public WorldGizmo {
public:
    ...
    // void cleanUpOldDrops(float deltaTime)
    // {
    //     for (auto& instance : instances)
    //     {
    //         instance.timeExisted += deltaTime;
    //     }
    //     auto it = std::remove_if(instances.begin(), instances.end(), [](DropInstance& instance) {
    ...
    void updateDropsToPhysicsSpots()
    {
        for (auto& instance : instances)
        {
            if (instance.body)
    ...
    void sendUpdatedInstancesList()
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, instances.size() * sizeof(DropInstance), instances.data(), GL_STATIC_DRAW);
        glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE, sizeof(DropInstance), (void*)0);
        glEnableVertexAttribArray(3);
        glVertexAttribDivisor(3, 1);

        glVertexAttribPointer(2, 1, GL_FLOAT, GL_FALSE, sizeof(DropInstance), (void*)sizeof(glm::vec4));
...
private:
    ...
    PxRigidStatic* dropCollCage = nullptr;
    ...
    void updateDropsCollisionCage(World* world)
    {
        UsableMesh mesh = {};
        ...
        if (lock != std::nullopt)
        {
            for (auto & drop : instances)
            {
                IntTup spot = IntTup(std::floor(drop.position.x), std::floor(drop.position.y + 0.5f), std::floor(drop.position.z));
            ...
            if(dropCollCage == nullptr)
            {
                dropCollCage = createStaticMeshCollider(PxVec3(0,0,0), mesh.positions, mesh.indices);
            } else
            {
                editStaticMeshCollider(dropCollCage, PxVec3(0,0,0), mesh.positions, mesh.indices);
            }
